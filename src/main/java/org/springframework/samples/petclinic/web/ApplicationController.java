...
public class ApplicationController {
	...
	@Autowired
	public ApplicationController(final ApplicationService applicationService, final AdoptionController adoptionController, final PetService petService, final UserService userService) {
	...
	@GetMapping(value = {
		"/applications/createApplicationForm/{petId}"
	})
	...
	public String listOfApplications(final Map<String, Object> model, final Principal p) {
	...
	public String processCreationForm(@Valid final Application application, final BindingResult result, @PathVariable("petId") final Integer idpet, final Principal p, final Map<String, Object> model) {
		final Pet pet = this.petService.findPetById(idpet);
		application.setPet(this.petService.findPetById(idpet));
	...
	public String rejectApplication(final Map<String, Object> model, @PathVariable("applicationId") final Integer applicationId, final Principal p) {

		final Optional<Application> application = this.applicationService.findById(applicationId);
	...
	public String aceptApplication(final Map<String, Object> model, @PathVariable("applicationId") final Integer applicationId, final Principal p) {
		...
		if (application.isPresent()) {
			...
			if (this.applicationService.isApplicationToMe(p.getName(), application.get())) {
				try {
					...
					model.put(this.message, e.toString());
				}
